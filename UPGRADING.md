...
This guide provides instructions for upgrading Gaia from v19.2.x. to v20.x.
...
Please use the correct release binary: `v20.0.0`.
...
Once a software upgrade governance proposal is submitted to the Cosmos Hub, both a reference to this proposal and an `UPGRADE_HEIGHT` are added to the [release notes](https://github.com/cosmos/gaia/releases/tag/v20.0.0).
...
For those validator and full node operators that are interested in ensuring preparedness for the impending upgrade, you can run a [v20 Local Testnet](https://github.com/cosmos/testnets/tree/master/local) or join in our [Cosmos Hub Public Testnet](https://github.com/cosmos/testnets/tree/master/public).

### Current runtime

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [Gaia v19.2.0](https://github.com/cosmos/gaia/releases/v19.2.0). We anticipate that operators who are running on v19.2.0, will be able to upgrade successfully. Validators are expected to ensure that their systems are up to date and capable of performing the upgrade. This includes running the correct binary and if building from source, building with the appropriate `go` version.

### Target runtime

The Cosmos Hub mainnet network, `cosmoshub-4`, will run **[Gaia v20.0.0](https://github.com/cosmos/gaia/releases/tag/v20.0.0)**. Operators _**MUST**_ use this version post-upgrade to remain connected to the network. The new version requires `go v1.22` to build successfully.
...
Make sure **Gaia v19.2.0** is installed by either downloading a [compatible binary](https://github.com/cosmos/gaia/releases/tag/v19.2.0), or building from source. Check the required version to build this binary in the `Makefile`.

Run Gaia v19.2.0 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v20" NEEDED at height: <UPGRADE_HEIGHT>: upgrade to v20 and applying upgrade "v20" at height:<UPGRADE_HEIGHT>
```

Stop the node, and switch the binary to **Gaia v20.0.0** and re-start by `gaiad start`.
...
- Create a `cosmovisor` folder inside `$GAIA_HOME` and move Gaia `v19.2.0` into `$GAIA_HOME/cosmovisor/genesis/bin`:
...
- Build Gaia `v20.0.0`, and move gaiad `v20.0.0` to `$GAIA_HOME/cosmovisor/upgrades/v20/bin`

```shell
mkdir -p  $GAIA_HOME/cosmovisor/upgrades/v20/bin
cp $(which gaiad) $GAIA_HOME/cosmovisor/upgrades/v20/bin
```
...
Steps to skip this upgrade proposal are simply to resume the cosmoshub-4 network with the (downgraded) v19.2.0 binary using the following command:
