...
func (mfd BypassMinFeeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
    ...
    if ctx.IsCheckTx() && !simulate && !(mfd.bypassMinFeeMsgs(msgs) && gas <= uint64(len(msgs))*maxBypassMinFeeMsgGasUsage) {
        ...
        if mfd.GlobalMinFee.Has(ctx, types.ParamStoreKeyMinGasPrices) {
            ...
            if !DenomsSubsetOf(feeCoins, requiredGlobalFees.Sort()) {
            ...
            if !IsAnyGTE(feeCoins, requiredGlobalFees.Sort()) {
                return ctx, sdkerrors.Wrapf(sdkerrors.ErrInsufficientFee, "insufficient global fees; got: %s required: %s", feeCoins, requiredGlobalFees)
            }
        }
        
        // passed globalfee check, this means the fee denom is in the globalfee denom,  check min gas price
        minGasPrices := ctx.MinGasPrices()
        // if not all coins are zero
        if !minGasPrices.IsZero() {
            ...
            // requiredFees here is ensured not all zero, but if min_gas_price=0stake,1photon, and feecoins is 0stake, it should not return err. so use IsAnyGTE() rather than IsAnyGTE() from sdk.
    ...
    // when the tx is bypass msg type, still need to check the denom is not random
...
// overwrite DenomsSubsetOf from sdk, to allow zero amt coins.
...
func IsAnyGTE(coins, coinsB sdk.Coins) bool {
    ...
    // if feecoins empty, and globalfee has one denom amt zero. feecoins equals to that 0denom.
    if len(coins) == 0 {
        // sdk.NewCoins will return non-zero coins
        //  coins = sdk.NewCoins(sdk.NewInt64Coin(coinsB.GetDenomByIndex(0),0))
