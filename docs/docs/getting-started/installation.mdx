...
**Note** : Depending on your Operating System, the golang toolchain might produce a binary with an extension. Please bear this in mind when looking at the above examples.

## Building with Docker

If you have a different version of the golang toolchain than the one required, you can either install and manage the different versions locally or you can use docker to run all the build commands. Below is an example of using Golang 1.21 Docker container.

```bash
# `pwd` is the current Gaia software directory, which is mounted on `/gaia-directory`.

docker run -it -v `pwd`:/gaia-directory golang:1.21 bash
root@5e9f5183ffe7:/go# cd /gaia-directory/
root@5e9f5183ffe7:/gaia-directory# ls
CHANGELOG.md	 README.md		 UPGRADING.md	client		go.mod		 sims.mk

# Run the commands / tooling that you want from here.
```
