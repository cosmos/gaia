...
This document describes the steps for validator and full node operators for the successful execution of the [Delta Upgrade](../roadmap/cosmos-hub-release-log.md#Delta-Upgrade), which adds the __Gravity DEX__ to the Cosmos Hub.
...
__IMPORTANT: PLEASE WAIT FOR THE BINARY TO HALT ON ITS OWN__. Do NOT shutdown the node yourself. If the node shuts down before the panic message, start the node and let it run until the panic stops the node for you.
