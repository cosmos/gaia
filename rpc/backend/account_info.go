...
func (b *Backend) GetBalance(address common.Address, blockNrOrHash rpctypes.BlockNumberOrHash) (*hexutil.Big, error) { //todo: JSON-RPC
	blockNum, err := b.BlockNumberFromTendermint(blockNrOrHash)
	if err != nil {
		return nil, err
	}
	...
	res, err := b.queryClient.BankClient.SpendableBalanceByDenom(b.ctx, req)
	if err != nil {
		return nil, err
	}

	val, ok := sdkmath.NewIntFromString(res.Balance.Amount.String())
	if !ok {
		return nil, errors.New("invalid balance")
	}
