...
func TestTokenizeSharesAndRedeemTokens(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			params, err := lsmKeeper.GetParams(ctx)
			require.NoError(t, err)
			...
			require.NoError(t, lsmKeeper.SetParams(ctx, params))

			// set the total liquid staked tokens
			lsmKeeper.SetTotalLiquidStakedTokens(ctx, math.ZeroInt())
			...
			require.NoError(t, err)
			err = lsmKeeper.SetLiquidValidator(ctx, lsmtypes.NewLiquidValidator(val1.OperatorAddress))
			require.NoError(t, err)
			...
			require.NoError(t, err)
			err = lsmKeeper.SetLiquidValidator(ctx, lsmtypes.NewLiquidValidator(val2.OperatorAddress))
			require.NoError(t, err)
			...
			resp, err := msgServer.TokenizeShares(ctx, &lsmtypes.MsgTokenizeShares{
			...
			require.Equal(t, lastRecordID+1, lsmKeeper.GetLastTokenizeShareRecordID(ctx))
			...
			newLiquidVal, err := lsmKeeper.GetLiquidValidator(ctx, addrVal1)
			require.NoError(t, err)

			// if the delegator was not a provider, check that the total liquid staked and validator liquid shares increased
			totalLiquidTokensAfterTokenization := lsmKeeper.GetTotalLiquidStakedTokens(ctx)
			...
			records := lsmKeeper.GetAllTokenizeShareRecords(ctx)
			...
			_, err = msgServer.RedeemTokensForShares(ctx, &lsmtypes.MsgRedeemTokensForShares{
				DelegatorAddress: delegatorAccount.String(),
			...
			totalLiquidTokensAfterRedemption := lsmKeeper.GetTotalLiquidStakedTokens(ctx)
...
func TestRedelegationTokenization(t *testing.T) {
	...
	_, validatorBAddress := setupTestTokenizeAndRedeemConversion(t, *lsmKeeper, *stakingKeeper, bankKeeper, ctx)
	...
	_, err = msgServer.TokenizeShares(ctx, &lsmtypes.MsgTokenizeShares{
		DelegatorAddress:    alice.String(),
	...
	require.Error(t, err)
	require.Equal(t, lsmtypes.ErrRedelegationInProgress, err)
	...
	shareRecord, err := lsmKeeper.GetTokenizeShareRecord(ctx, lsmKeeper.GetLastTokenizeShareRecordID(ctx))
...
func setupTestTokenizeAndRedeemConversion(
	...
	require.NoError(t, err)
	liquidVal := lsmtypes.NewLiquidValidator(validatorAddress.String())
...
func TestTokenizeAndRedeemConversion_SlashBeforeDelegation(t *testing.T) {
	...
	var (
		stakingKeeper = f.stakingKeeper
		bankKeeper    = f.bankKeeper
		lsmKepeer     = f.lsmKeeper
	)
	skServer := stakingkeeper.NewMsgServerImpl(stakingKeeper)
	lsmServer := lsmkeeper.NewMsgServerImpl(lsmKepeer)
	delegatorAddress, validatorAddress := setupTestTokenizeAndRedeemConversion(t, *lsmKepeer, *stakingKeeper,
		bankKeeper, ctx)
	...
	_, err = lsmServer.TokenizeShares(ctx, &lsmtypes.MsgTokenizeShares{
		DelegatorAddress:    delegatorAddress.String(),
		ValidatorAddress:    validatorAddress.String(),
		Amount:              delegateCoin,
	...
	_, err = lsmServer.RedeemTokensForShares(ctx, &lsmtypes.MsgRedeemTokensForShares{
		DelegatorAddress: delegatorAddress.String(),
		Amount:           shareToken,
	})
...
func TestTokenizeAndRedeemConversion_SlashBeforeTokenization(t *testing.T) {
	...
	msgServer := lsmkeeper.NewMsgServerImpl(lsmKeeper)
	skServer := stakingkeeper.NewMsgServerImpl(stakingKeeper)
	delegatorAddress, validatorAddress := setupTestTokenizeAndRedeemConversion(t, *lsmKeeper, *stakingKeeper,
		bankKeeper, ctx)
...
func TestTransferTokenizeShareRecord(t *testing.T) {
	...
	err = lsmKeeper.AddTokenizeShareRecord(ctx, lsmtypes.TokenizeShareRecord{
		Id:            1,
	...
	_, err = msgServer.TransferTokenizeShareRecord(ctx, &lsmtypes.MsgTransferTokenizeShareRecord{
	...
	record, err := lsmKeeper.GetTokenizeShareRecord(ctx, 1)
	require.NoError(t, err)
	require.Equal(t, record.Owner, addrAcc2.String())

	records := lsmKeeper.GetTokenizeShareRecordsByOwner(ctx, addrAcc1)
	require.Len(t, records, 0)
	records = lsmKeeper.GetTokenizeShareRecordsByOwner(ctx, addrAcc2)
	require.Len(t, records, 1)
}
...
func TestEnableDisableTokenizeShares(t *testing.T) {
	...
	require.NoError(t, lsmKeeper.SetLiquidValidator(ctx, lsmtypes.NewLiquidValidator(validator.OperatorAddress)))
	...
	tokenizeMsg := lsmtypes.MsgTokenizeShares{
		DelegatorAddress:    delegatorAddress.String(),
		ValidatorAddress:    validatorAddress.String(),
		Amount:              stakeToken,
		TokenizedShareOwner: delegatorAddress.String(),
	}
	redeemMsg := lsmtypes.MsgRedeemTokensForShares{
		DelegatorAddress: delegatorAddress.String(),
	}
	disableMsg := lsmtypes.MsgDisableTokenizeShares{
		DelegatorAddress: delegatorAddress.String(),
	}
	enableMsg := lsmtypes.MsgEnableTokenizeShares{
		DelegatorAddress: delegatorAddress.String(),
	}
	...
	require.ErrorIs(t, err, lsmtypes.ErrTokenizeSharesAlreadyEnabledForAccount)
	...
	_, err = msgServer.DisableTokenizeShares(ctx, &disableMsg)
	require.ErrorIs(t, err, lsmtypes.ErrTokenizeSharesAlreadyDisabledForAccount)

	// Attempt to tokenize, it should fail since tokenization is disabled
	_, err = msgServer.TokenizeShares(ctx, &tokenizeMsg)
	require.ErrorIs(t, err, lsmtypes.ErrTokenizeSharesDisabledForAccount)
	...
	authorizations := lsmKeeper.GetPendingTokenizeShareAuthorizations(ctx, unlockTime)
	...
	_, err = lsmKeeper.RemoveExpiredTokenizeShareLocks(ctx, ctx.BlockTime())
	require.NoError(t, err)
...
func TestTokenizeAndRedeemVestedDelegation(t *testing.T) {
	...
	val1.Status = stakingtypes.Bonded
	require.NoError(t, stakingKeeper.SetValidator(ctx, val1))
	require.NoError(t, lsmKeeper.SetLiquidValidator(ctx, lsmtypes.NewLiquidValidator(val1.OperatorAddress)))
	require.NoError(t, stakingKeeper.SetValidatorByPowerIndex(ctx, val1))
	err = stakingKeeper.SetValidatorByConsAddr(ctx, val1)
	require.NoError(t, err)
	...
	_, err = msgServer.RedeemTokensForShares(ctx,
		&lsmtypes.MsgRedeemTokensForShares{
			DelegatorAddress: addrAcc1.String(),
