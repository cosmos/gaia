...
func (s *PermissionlessConsumersSuite) TestRewardsWhitelisting() {
	validators := 1
	fullNodes := 0
	...
	proposal := cosmos.SoftwareUpgradeProposal{
		Deposit:     "5000000" + chainsuite.Ucon, // greater than min deposit
		Title:       "Changeover",
		Name:        "sovereign-changeover",
		Description: "Changeover",
		Height:      int64(initialHeight) - 1,
	}
	...
	genesis, err := consumer.ExportState(s.GetContext(), currentHeight)
	s.Require().NoError(err)

	ccvState, _, err := s.Chain.GetNode().ExecQuery(s.GetContext(), "provider", "consumer-genesis", consumerID)
	s.Require().NoError(err)
	genesis, err = sjson.SetRaw(genesis, "app_state.ccvconsumer", string(ccvState))
	s.Require().NoError(err)

	genesis, err = sjson.Set(genesis, "app_state.slashing.params.signed_blocks_window", strconv.Itoa(chainsuite.SlashingWindowConsumer))
	s.Require().NoError(err)
	genesis, err = sjson.Set(genesis, "app_state.ccvconsumer.params.reward_denoms", []string{chainsuite.Ucon})
	s.Require().NoError(err)
	genesis, err = sjson.Set(genesis, "app_state.ccvconsumer.params.provider_reward_denoms", []string{s.Chain.Config().Denom})
	s.Require().NoError(err)
	genesis, err = sjson.Set(genesis, "app_state.ccvconsumer.params.blocks_per_distribution_transmission", chainsuite.BlocksPerDistribution)
	s.Require().NoError(err)
	...
	_, err = s.Chain.GetNode().ExecTx(s.GetContext(), s.Chain.ValidatorWallets[0].Moniker, "provider", "set-consumer-commission-rate", consumerID, "0.5")
	s.Require().NoError(err)
	_, err = s.Chain.Validators[0].ExecTx(s.GetContext(), s.Chain.ValidatorWallets[0].Moniker, "distribution", "withdraw-rewards", s.Chain.ValidatorWallets[0].ValoperAddress, "--commission")
	s.Require().NoError(err)
	s.Require().NoError(testutil.WaitForBlocks(s.GetContext(), 1, s.Chain, consumer))

	consumerUpdate := &providertypes.MsgUpdateConsumer{
		ConsumerId: consumerID,
		Metadata: &providertypes.ConsumerMetadata{
			Name:        consumer.Config().Name,
			Description: "Consumer chain",
	...
	consumerUpdateBz, err := json.Marshal(consumerUpdate)
	s.Require().NoError(err)
	consumerUpdateBz, err = sjson.SetRawBytes(consumerUpdateBz, "allowlisted_reward_denoms", []byte(fmt.Sprintf(`{"denoms": ["%s"]}`, rewardDenom)))
	s.Require().NoError(err)
	err = s.Chain.GetNode().WriteFile(s.GetContext(), consumerUpdateBz, "consumer-update.json")
	s.Require().NoError(err)
	_, err = s.Chain.GetNode().ExecTx(s.GetContext(), interchaintest.FaucetAccountKeyName,
		"provider", "update-consumer", path.Join(s.Chain.GetNode().HomeDir(), "consumer-update.json"))
	s.Require().NoError(err)
