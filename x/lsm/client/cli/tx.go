...
// NewTxCmd returns a root CLI command handler for all x/lsm transaction commands.
...
Example:
$ %s tx lsm tokenize-share %s1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj 100stake %s1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj --from mykey
`,
...
Example:
$ %s tx lsm redeem-tokens 100sharetoken --from mykey
`,
				version.AppName,
			),
		),
...
Example:
$ %s tx lsm transfer-tokenize-share-record 1 %s1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj --from mykey
`,
...
Example:
$ %s tx lsm disable-tokenize-shares --from mykey
`, version.AppName),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
...
Example:
$ %s tx lsm enable-tokenize-shares --from mykey
`, version.AppName),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
